...
var $, DEFAULT_CREL_EXPR, SPACES_ONLY, Serialiser, TEXT_LEADING_WHITESPACE, TEXT_TRAILING_WHITESPACE, WHITESPACE_ONLY, componentClassTagConvention, containsNewlines, entityDecode, exports, find, firstNonWhitespaceChild, genericBranchSerialiser, genericLeafSerialiser, joinList, last, makeCreateElementTemplate, nodeSerialisers, ref, serialise, stringEscape;
...
module.exports = exports = serialise = function(parseTree, opts) {
  return new Serialiser().serialise(parseTree, opts);
};

DEFAULT_CREL_EXPR = '`${REACT}.createElement(${ELEMENT}, ${ARGS})`';

makeCreateElementTemplate = function(pattern) {
  eval("function template(REACT, ELEMENT, ARGS) {\n  return " + pattern + ";\n}");
  return template;
};
...
Serialiser = (function() {
  ...
  Serialiser.prototype.serialise = function(parseTree, opts1) {
    var domObjectParts, serialised;
    this.opts = opts1 != null ? opts1 : {};
    this.hasJSX = false;
    this.jsxExpression = makeCreateElementTemplate(this.opts.jsxExpression || DEFAULT_CREL_EXPR);
    ...
    serialised = this.serialiseNode(parseTree);
    if (this.opts.jsxImport && this.hasJSX) {
      return this.opts.jsxImport + "\n" + serialised;
    } else {
      return serialised;
    }
  };
...
nodeSerialisers = {
  ...
  CJSX_EL: function(node) {
    ...
    this.hasJSX = true;
    return this.jsxExpression(this.reactObject, element, joinList(serialisedChildren));
  },
